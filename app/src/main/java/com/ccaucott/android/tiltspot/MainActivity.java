...
package com.ccaucott.android.tiltspot;
...
public class MainActivity extends AppCompatActivity implements SensorEventListener {
    ...
    //  to hold copies of the accelerometer and magnetometer data.
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (mSensorMagnetometer == null){
            mSensorMagnetometer = mSensorManager.getDefaultSensor(Sensor.TYPE_GYROSCOPE);
        }
    ...
    public void onSensorChanged(SensorEvent sensorEvent) {
        ...
        /*
        The getRotationMatrix() method returns a boolean (the rotationOK variable), which is true if the rotation was successful.
        The boolean might be false if the device is free-falling (meaning that the force of gravity is close to 0),
        or if the device is pointed very close to magnetic north. The incoming sensor data is unreliable in these cases,
        and the matrix can't be calculated. Although the boolean value is almost always true, it's good practice to check that value anyhow.
        */
        float[] rotationMatrix = new float[9];
        boolean rotationOk = SensorManager.getRotationMatrix(rotationMatrix, null, mAccelerometerData, mMagnetometerData);

        // to hold the new adjusted rotation matrix.
